package constants.locators;

public class HeaderPageConstants {
    public static final String SEARCH_FIELD = "search";
    public static final String SEARCH_BUTTON = "//*[@id=\"search_mini_form\"]/button";
    public static final String SEARCH_CATEGORY_DROPDOWN_BUTTON = "dropdown-toggle";
    public static final String SEARCH_SUGGESTIONS = "search_autocomplete";
    public static final String SEARCH_SUGGESTION_ITEM = "qs-option-name";
    public static final String EXTENDED_SEARCH_BUTTON = "search_link";
    public static final String CART_ITEMS_COUNT = "count_block";
    public static final String CART_BUTTON = "shopcart_block";
    public static final String MESSAGE = "div[data-bind='html: message.text']";

}
